...
public class ItemHandler extends RSBuddyCommon
{
    ...
    public static void bind(int itemID) {
        ...
        if (item != null) {
            while (!Menu.click("Bind") && !MyInventory.getItem(itemID).getName().contains("(b)")) {
                Mouse.move((int)item.getComponent().getBoundingRect().getCenterX(), (int)item.getComponent().getBoundingRect().getCenterY());
     ...
     *
     * @param equipable      the Equipable to be checked
     * @param EquipmentIndex the equipment spot it goes in
     * @return true if should be equiped
     */
    public static boolean shouldEquip(Equipable equipable, int EquipmentIndex) {
        Equipable toEquip = equipable;
        ...
        if (toEquip.getStyle() ==
            MyPlayer.combatStyle() &&
            toEquip.canWield() && toEquip.getEquipmentIndex() == EquipmentIndex) {
