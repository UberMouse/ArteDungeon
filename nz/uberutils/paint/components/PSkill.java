...
public class PSkill extends PComponent
{
    ...
    public void drawProgressBar(Graphics g, int skill, int x, int y, int w, int h, boolean hover) {
        ...
        g2.fillRect(x, y, w, h);
        g2.setPaint(grad2);
        g2.fillRect(x, y, (int) (w * (Skills.getPercentToNextLevel(skill) / 100.0)), h);
        g2.setColor(scheme.text);
        g2.drawRect(x, y, w, h);
        g2.setFont(new Font("Arial", 0, 9));
        String progress = Skills.getPercentToNextLevel(skill)
                          + "% to " + (Skills.getRealLevel(skill) + 1)
                          + " " + Skills.SKILL_NAMES[skill] + " | "
                          + Skills.getExpToNextLevel(skill)
                          + " XPTL";
